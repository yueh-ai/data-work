...
type ComparePayload = {
  sessionId: string;
  previous: UploadDescriptor | null;
  current: UploadDescriptor | null;
  delta: StructuralDelta | null;
};

type StructuralDelta = {
  rowCount: CountDelta;
  columnCount: CountDelta;
...
function SessionView({ sessionId }: { sessionId: string }) {
  ...
  useEffect(() => {
    ...
    events.addEventListener("compare", (event) => {
      setCompare(JSON.parse((event as MessageEvent).data) as ComparePayload);
    });
  ...
  useEffect(() => {
    ...
    async function loadCompare() {
      try {
        ...
        // SSE remains the primary live channel; the fetch is only refresh hydration.
      }
    }
  ...
  return (
    <main className="app-shell">
      ...
      <section className="metrics-grid" aria-label="Current CSV metadata">
        <Metric label="Rows" value={compare?.current ? formatNumber(compare.current.summary.rowCount) : table ? formatNumber(table.rows.length) : "0"} />
        <Metric
          label="Columns"
          value={
            compare?.delta
              ? `${formatNumber(compare.delta.columnCount.previous)} -> ${formatNumber(compare.delta.columnCount.current)}`
              : compare?.current
                ? formatNumber(compare.current.summary.columnCount)
                : table
                  ? formatNumber(table.columns.length)
                  : "0"
          }
        />
      ...
      {compare?.current ? (
        <CompareWorkspace compare={compare} />
      ) : table ? (
...
function CompareWorkspace({ compare }: { compare: ComparePayload }) {
  ...
  if (!previous || !compare.delta) {
    return <SingleVersionPreview upload={current} />;
  }

  return (
    <section className="compare-workspace" aria-label="Before and after CSV comparison">
      <div className="compare-header">
        <div>
          <p className="eyebrow">Inspectable Compare</p>
          <h2>Previous vs current upload</h2>
        </div>
        <div className="version-pair" aria-label="Compared versions">
          <span>v{previous.version}</span>
          <span>v{current.version}</span>
        </div>
      </div>

      <div className="delta-grid" aria-label="Structural changes">
        <DeltaCard label="Rows" delta={compare.delta.rowCount} />
        <DeltaCard label="Columns" delta={compare.delta.columnCount} />
        <ChangeList label="Removed Columns" values={compare.delta.removedColumns} tone="removed" />
        <ChangeList label="Added Columns" values={compare.delta.addedColumns} tone="added" />
      </div>

      <ColumnDiffPanel previous={previous.summary} current={current.summary} delta={compare.delta} />

      <div className="split-preview">
        <SampleTable title="Previous Upload" upload={previous} delta={compare.delta} side="previous" />
        <SampleTable title="Current Upload" upload={current} delta={compare.delta} side="current" />
      </div>
    </section>
  );
}

function SingleVersionPreview({ upload }: { upload: UploadDescriptor }) {
  return (
    <section className="compare-workspace" aria-label="Current CSV sample">
      <div className="compare-header">
        <div>
          <p className="eyebrow">Inspectable Compare</p>
          <h2>Upload one more version to compare changes</h2>
        </div>
        <div className="version-pair" aria-label="Current version">
          <span>v{upload.version}</span>
        </div>
      </div>
      <SampleTable title="Current Upload" upload={upload} side="current" />
    </section>
  );
}

function DeltaCard({ label, delta }: { label: string; delta: CountDelta }) {
  return (
    <div className="delta-card">
      <span>{label}</span>
      <strong>
        {formatNumber(delta.previous)} {"->"} {formatNumber(delta.current)}
      </strong>
      <small className={delta.changed === 0 ? "delta-neutral" : delta.changed < 0 ? "delta-removed" : "delta-added"}>
        {delta.changed === 0 ? "No change" : `${delta.changed > 0 ? "+" : ""}${formatNumber(delta.changed)}`}
      </small>
    </div>
  );
}

function ChangeList({ label, values, tone }: { label: string; values: string[]; tone: "added" | "removed" }) {
  return (
    <div className="delta-card">
      <span>{label}</span>
      <strong>{values.length ? values.join(", ") : "None"}</strong>
      <small className={tone === "removed" ? "delta-removed" : "delta-added"}>
        {values.length ? `${formatNumber(values.length)} ${tone}` : "No change"}
      </small>
    </div>
  );
}

function ColumnDiffPanel({ previous, current, delta }: { previous: CsvSummary; current: CsvSummary; delta: StructuralDelta }) {
  const orderedColumns = [...previous.columns, ...current.columns.filter((column) => !previous.columns.includes(column))];

  return (
    <div className="column-diff-panel" aria-label="Column-level changes">
      <div className="panel-title">
        <h2>Column Diff</h2>
        <p>
          {formatNumber(delta.retainedColumns.length)} retained, {formatNumber(delta.removedColumns.length)} removed,{" "}
          {formatNumber(delta.addedColumns.length)} added
        </p>
      </div>
      <div className="column-chip-row">
        {orderedColumns.map((column) => {
          const state = delta.removedColumns.includes(column) ? "removed" : delta.addedColumns.includes(column) ? "added" : "retained";
          return (
            <span key={column} className={`column-chip column-chip--${state}`}>
              {column}
              <small>{state}</small>
            </span>
          );
  ...
  return (
    <div className="sample-panel">
      <div className="sample-heading">
        <div>
          <h2>{title}</h2>
          <p>{upload.filename ?? "working.csv"}</p>
        </div>
        <span>{formatNumber(upload.summary.rowCount)} rows</span>
      </div>
      <div className="table-scroll table-scroll--sample">
        <table className="sample-table">
          <thead>
            <tr>
              <th className="row-number">#</th>
              {upload.summary.columns.map((column) => (
                <th key={column} className={columnStateClass(column, side, delta)}>
                  <span>{column}</span>
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {upload.summary.sampleRows.map((row, index) => (
              <tr key={index}>
                <td className="row-number">{index + 1}</td>
                {upload.summary.columns.map((column, columnIndex) => (
                  <td key={column} className={columnStateClass(column, side, delta)}>
                    {row[columnIndex]}
                  </td>
                ))}
              </tr>
...
function columnStateClass(column: string, side: "previous" | "current", delta?: StructuralDelta) {
  if (!delta) {
    return "";
  }
  if (side === "previous" && delta.removedColumns.includes(column)) {
    return "is-removed-column";
  }
  if (side === "current" && delta.addedColumns.includes(column)) {
    return "is-added-column";
  }
  return "";
}
