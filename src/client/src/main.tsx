...
import { buildDataStory, type DataStory } from "./csvStory.js";
import "./styles.css";
...
type StoryPhase = "received" | "focus" | "change" | "settled";
type StoryPlayback = {
  id: number;
  previous: ParsedTable | null;
  current: ParsedTable;
  story: DataStory;
};
...
function SessionView({ sessionId }: { sessionId: string }) {
  ...
  const [playback, setPlayback] = useState<StoryPlayback | null>(null);
  const [storyPhase, setStoryPhase] = useState<StoryPhase>("settled");
  const [parseError, setParseError] = useState<string | null>(null);
  const [copyState, setCopyState] = useState<string | null>(null);
  const [uploading, setUploading] = useState(false);
  const tableRef = useRef<ParsedTable | null>(null);
  ...
  useEffect(() => {
    ...
    events.addEventListener("csv", (event) => {
      ...
      const nextTable = parseCsv(next.csv);
      setCsvEvent(next);
      if (!nextTable) {
        return;
      }
      ...
      const story = buildDataStory(previous ? toStoryShape(previous) : null, toStoryShape(nextTable));
      tableRef.current = nextTable;
      setTable(nextTable);
      setPlayback({
        id: (playbackIdRef.current += 1),
        previous,
        current: nextTable,
        story
      });
  ...
  useEffect(() => {
    ...
    setStoryPhase("received");
    const focusTimer = window.setTimeout(() => setStoryPhase("focus"), 800);
    const changeTimer = window.setTimeout(() => setStoryPhase("change"), 2600);
    const settleTimer = window.setTimeout(() => setStoryPhase("settled"), 12_000);
  ...
  return (
    <main className="app-shell">
      ...
      {table ? (
        <TablePreview playback={playback} phase={storyPhase} table={table} filename={csvEvent?.filename} />
      ) : (
...
function TablePreview({
  ...
  playback: StoryPlayback | null;
  phase: StoryPhase;
  table: ParsedTable;
  filename?: string | null;
}) {
  const story = playback?.story ?? buildDataStory(null, toStoryShape(table));
  const display = buildDisplayTable(playback, phase, table);
  const previewRows = display.rows.slice(0, 64);

  return (
    <section className={`preview-panel preview-panel--${phase} preview-panel--${story.kind}`} aria-label="Current CSV table">
      <div className="preview-heading">
        <div>
          <h2>{filename ?? "Current Working CSV Version"}</h2>
          <p>{table.columns.map((column) => `${column}: ${table.types[column]}`).join(" · ")}</p>
        </div>
        <div className="story-cue" aria-live="polite">
          <span>{storyStageLabel(phase, story)}</span>
      ...
      <div className="table-scroll story-stage">
        <div className="story-spotlight" aria-hidden="true" />
        <table>
          <thead>
            <tr>
              <th className="row-number">#</th>
              {display.columns.map((column) => (
                <th key={column.name} className={columnClass(column, story)}>
          ...
          <tbody>
            {previewRows.map((row, index) => (
              <tr key={index}>
                ...
                {display.columns.map((column) => (
                  <td key={column.name} className={columnClass(column, story)}>
        ...
        {display.rows.length > previewRows.length ? (
          <div className="preview-limit">Showing first {previewRows.length.toLocaleString()} rows in the animated window.</div>
        ) : null}
      </div>
    </section>
  );
...
function buildDisplayTable(playback: StoryPlayback | null, phase: StoryPhase, table: ParsedTable) {
  if (!playback?.previous || phase === "settled") {
    return {
      columns: table.columns.map((column) => ({ name: column, type: table.types[column], status: "current" as const })),
      rows: table.rows
    };
  }

  const removed = new Set(playback.story.removedColumns);
  const added = new Set(playback.story.addedColumns);
  const previousOnlyColumns = playback.previous.columns.filter((column) => removed.has(column));
  const columns = [...playback.current.columns];

  for (const removedColumn of previousOnlyColumns) {
    const previousIndex = playback.previous.columns.indexOf(removedColumn);
    columns.splice(Math.min(previousIndex, columns.length), 0, removedColumn);
  }

  return {
    columns: columns.map((column) => ({
      name: column,
      type: playback.previous?.types[column] ?? playback.current.types[column] ?? "text",
      status: removed.has(column) ? ("removed" as const) : added.has(column) ? ("added" as const) : ("current" as const)
    })),
    rows: playback.current.rows.map((row, index) => ({
      ...(playback.previous?.rows[index] ?? {}),
      ...row
    }))
  };
}

function columnClass(column: DisplayColumn, story: DataStory) {
  const classes = [`story-column--${column.status}`];
  if (story.focusColumns.includes(column.name)) {
    classes.push("story-column--focus");
  }
  return classes.join(" ");
}

function storyStageLabel(phase: StoryPhase, story: DataStory) {
  if (story.kind === "initial") {
  ...
  if (phase === "focus") {
    return "Focusing changed region";
  }
  if (phase === "change") {
    return "Morphing structure";
  }
  return "Settled";
}

function toStoryShape(table: ParsedTable) {
  return {
...
function buildWarnings(columns: string[], rows: Record<string, string>[]) {
  ...
  if (rows.length > 10_000) {
    warnings.push("Large CSV: parsing the full file, animating the first 64 preview rows.");
  }
  return warnings;
}
